...
export interface CodingTask {
  id: string;
  title: string;
  category: 'pe-app' | 'project-27' | 'productivity' | 'new-projects';
...
const CATEGORIES = [
  { id: 'pe-app' as const, label: 'PE App', color: 'var(--color-pe-app)', rgb: 'var(--color-pe-app-rgb)' },
  { id: 'project-27' as const, label: 'Project 27', color: 'var(--color-project27)', rgb: 'var(--color-project27-rgb)' },
  { id: 'productivity' as const, label: 'Productivity', color: 'var(--color-coding-prod)', rgb: 'var(--color-coding-prod-rgb)' },
  { id: 'new-projects' as const, label: 'New Projects', color: 'var(--color-new-projects)', rgb: 'var(--color-new-projects-rgb)' },
];

export const TabCodingProjects: React.FC<TabCodingProjectsProps> = ({
  tasks,
  setTasks,
  prioritiesWeek,
  setPrioritiesWeek,
  ...
  const handleRowDrop = (
    ...
    if (payload.kind === 'task') {
      setTasks(prev => {
        ...
        if (targetTask?.id === source.id) return prev;
  ...
  const [bulkCategory, setBulkCategory] = useState<CodingCategory>('pe-app');
  const [bulkTimeframe, setBulkTimeframe] = useState<Timeframe>('near');
  ...
  const handleBulkImport = () => {
    ...
    const newTasksList: CodingTask[] = lines.map(line => ({
      id: crypto.randomUUID(),
      title: line,
  ...
  const handleToggleClick = (task: CodingTask) => {
    if (task.isCompleted) {
      toggleTask(task.id);
      return;
    }
  ...
  const handleAddTask = (category: CodingCategory, timeframe: Timeframe) => {
  ...
  // Copy before sorting to avoid mutating the derived filteredHistory array.
  ...
  const renderMiniCalendar = (monthOffset: number) => {
    ...
    days.forEach((day, idx) => {
      ...
      if (tempWeek.length === 7 || idx === days.length - 1) {
        while (tempWeek.length < 7) tempWeek.push(null);
        weeks.push(tempWeek);
        tempWeek = [];
      }
    });
          ...
          <div className="today-header-shrunk">
            <div className="today-title-shrunk">
              ...
              <span className="today-date-text">
                Coding Projects — {new Date().toLocaleDateString('en-US', {
                  weekday: 'short',
                  month: 'short',
                  day: 'numeric',
                  year: 'numeric',
            ...
            <div className="bulk-import-form">
              <div className="bulk-import-controls">
                <div className="control-group">
                  ...
                  <select
                    value={bulkCategory}
                    onChange={(e) => setBulkCategory(e.target.value as CodingCategory)}
                ...
                <div className="control-group">
                  ...
                  <select
                    value={bulkTimeframe}
                    onChange={(e) => setBulkTimeframe(e.target.value as Timeframe)}
                    className="bulk-select"
                  >
                ...
                >
                  {targets.map((task, bucketIdx) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`task-item task-${cat.id} target-item-row`}
                ...
                >
                  {nearTerm.map((task, bucketIdx) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`task-item task-item-with-actions task-${cat.id} ${completingTaskId === task.id ? 'task-item-completing' : ''}`}
                          title={`Added ${task.dateAdded}`}
                          draggable={editingTaskId !== task.id}
                          onDragStart={(e) =>
                            e.dataTransfer.setData(
            ...
            <thead>
              <tr>
                ...
                <th className="sortable-th" onClick={() => handleSort('category')} style={{ cursor: 'pointer' }}>
                  Project {sortBy === 'category' ? (sortOrder === 'asc' ? '▲' : '▼') : ''}
                </th>
